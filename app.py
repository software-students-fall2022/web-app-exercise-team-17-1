...
def delete():
    """
    Route for POST requests to delete contact.
    Accepts the login information and saves in the database.
    """
    if request.method == "GET":
        ...
        db.contactList.delete_one(doc1)  # delete the contact
        return render_template('create.html')
...
def contacts_list():
    ...
    """
    docs = db.contactList.find({"currentUser": currentUser}).sort("created_at",
                                                                  -1)  # sort in descending order of created_at timestamp
